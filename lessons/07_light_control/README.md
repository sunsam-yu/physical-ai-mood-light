...
Part 6의 자신의 Arduino와 웹 파일을 그대로 이어서 사용합니다.

1. 웹에 색상·밝기·모드 입력 요소와 `조명 적용` 버튼을 추가합니다.
2. 버튼 한 개가 `LIGHT,R,G,B,밝기,모드` 한 줄을 만드는 함수를 작성합니다.
3. 자신의 Arduino 코드에서 `readWebCommand()` 처리를 확장합니다.
4. `handleWebCommand()`에서 숫자 범위를 검사한 뒤 `applyLight()`를 호출합니다.
5. 실제 적용값을 `sendLightState()`로 웹에 돌려보냅니다.
6. 웹은 보낸 명령이 아니라 `light_state` 응답을 받은 뒤 디지털 트윈을 바꿉니다.

각 단계마다 먼저 고정된 파랑·밝기 20 한 가지로 시험하고, 정상 작동한 뒤 입력 요소와 연결합니다.
...
1. Part 6까지 누적한 자신의 Arduino 파일에 명령 해석과 조명 응답 함수를 추가합니다.
2. 활동을 마친 뒤 `07_light_control.ino`와 함수별로 비교합니다.
3. 업로드가 끝나면 시리얼 모니터를 닫습니다.
4. 웹에서 전송 방법을 `Arduino USB(Web Serial)`로 바꾸고 UNO 포트를 선택합니다.
...
- [Part 7 Arduino 명령 처리 모범답안](../../arduino/07_light_control/07_light_control.ino)
- [Part 7 웹 조명 제어 모범답안](../../web/07_light_control)

독립 실행본을 그대로 교체하지 말고 명령 생성, 명령 해석, `applyLight()`, `sendLightState()`, 응답 후 화면 갱신 부분만 자신의 누적 파일과 비교합니다.

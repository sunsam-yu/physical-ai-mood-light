...
Part 7의 대시보드에 AI 영역을 추가합니다. MediaPipe 모델 준비와 카메라 초기화 코드는 제공 블록을 지정 위치에 붙이고, 학습의 핵심인 특징 표시·샘플 수집·분류·결과 표시 함수는 순서대로 연결합니다.

1. AI 상태와 웹캠 영역의 HTML을 추가합니다.
2. 제공된 모델·카메라 초기화 블록을 붙이고 웹캠 영상만 확인합니다.
3. `updateFeatureDisplay()`를 작성해 특징값을 막대로 표시합니다.
4. `collectSamples(label)`을 연결해 한 범주씩 샘플을 모읍니다.
5. `showPrediction()`을 연결해 현재 판정과 안정된 최근 결과를 표시합니다.
6. 기존 센서 카드와 수동 조명이 계속 작동하는지 함께 확인합니다.

AI 페이지를 별도로 완성해 갈아 끼우지 않습니다. 기존 웹 대시보드에 AI 입력 기능을 한 블록씩 붙입니다.
...
- [Part 8 AI 기능 모범답안](../../web/08_expression_ai)
- [특징 추출·KNN 분류 함수](../../web/08_expression_ai/classifier.mjs)

라이브러리 초기화 코드는 제공 블록과 일치하는지 확인하고, 학생이 연결한 특징 표시·샘플 수집·분류 함수와 이벤트 호출 위치를 중심으로 비교합니다.

...
이 자료는 Arduino UNO, 센서 네 종류, 네오픽셀, 웹캠, 웹 기술을 하나씩 연결해 **Physical AI 무드등**을 완성하는 실습 과정입니다. 학생은 Part마다 새 완성 파일을 실행하지 않습니다. Part 2에서 만든 Arduino 파일과 Part 5에서 만든 웹 파일에 함수와 화면 요소를 직접 타이핑하며 Part 10까지 누적합니다.
...
| [Part 2](02_component_test/README.md) | 키트 조립과 코드 뼈대 | 선언·핀 설정·네오픽셀 초기화 | 상수, 객체, `setup()`, `loop()` |
| [Part 3](03_sensor_basics/README.md) | 센서 입력 함수 누적 | 같은 파일에 센서 함수 4종 추가 | 아날로그·디지털 입력, 거리, 함수 호출 |
| [Part 4](04_modular_controller/README.md) | 상태와 출력 함수 누적 | 입력·결정·출력이 함께 도는 코드 | 함수, 상태, `millis()` |
...
1. 이전 Part의 자신의 파일을 열고 정상 작동을 먼저 확인합니다.
2. 이번 함수나 화면 요소를 한 블록씩 직접 타이핑합니다.
3. 함수 작성 뒤 `setup()`·`loop()`·이벤트에 호출을 연결하고 즉시 실행합니다.
4. 한 줄이나 한 값을 직접 바꾸어 결과를 비교한 뒤 체크포인트를 저장합니다.
5. Part 마지막에만 GitHub 모범답안을 열고 함수와 호출 위치를 비교합니다.
6. Arduino IDE의 시리얼 모니터와 웹페이지는 같은 포트를 동시에 사용할 수 없습니다.
7. 웹페이지와 실제 장치가 다르게 보이면 입력 → 판단 → 명령 → 실제 응답 순서로 확인합니다.
8. 네오픽셀 밝기는 USB 전원을 고려해 `0~80` 범위에서 사용합니다.
9. 얼굴 영상은 저장하거나 전송하지 않습니다. 결과는 실제 감정이 아니라 화면에 나타난 표정 특징의 분류입니다.

GitHub의 단계별 코드는 수업 시작 파일이 아니라 그 Part까지 마친 뒤 보는 모범답안입니다. 학생은 자신의 파일을 덮어쓰지 않고, 빠진 함수·호출문·데이터 형식만 찾아 수정합니다.
